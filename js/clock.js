...
setInterval(function() {
    ...
    if (second == 0) {
        ...
        } else if (snoopList.includes(currentTime)) {
...
const addTime = (ev)=> {
    ev.preventDefault(); 
    let selected = document.querySelector('input[type="time"]');
    let selectedTime = selected.value;
    timesList.push(selectedTime);
    document.getElementById('timeSelector').value = null;

    console.warn('added', {timesList} );

    // saving to local storage
    localStorage.setItem('MyTimesList', JSON.stringify(timesList) );
